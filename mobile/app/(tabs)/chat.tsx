...
export default function ChatScreen() {
  ...
  const sendText = useCallback(
    async (text: string) => {
      ...
      try {
        const { reply, suggestedPrompts: next, contextLevel: level } = await aiApi.chat(text.trim());
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.inputRow}>
          <TextInput
            ...
            maxLength={4000}
          />
          <Button title="↑" onPress={send} loading={loading} disabled={!input.trim()} style={styles.sendBtn} />
        </View>
